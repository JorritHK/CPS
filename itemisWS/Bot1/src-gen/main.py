...
from grid.util import Callback as UtilCallback, Coord
...
class SCTConnect:
    ...
    def __init__(self):
        ...
        # setup callback
        self.sm.internal_operation_callback = UtilCallback()
    ...
    def run(self):
        while not self.sm.output.finish:
            ...
            self.node.vel_publish(
                x=self.sm.output.speed, rz=self.sm.output.rotation
            )
            ...
            print(
                f"Wall front: {self.sm.grid.wall_front}, right: {self.sm.grid.wall_right}, "
    ...
    def get_data_laser(self):
        ...
        if self.sm.start_pos.laser_deg_offset < 0:
            degrees_directions = [
                360 + self.sm.start_pos.laser_deg_offset,
            ...
            degrees_directions = [
                0 + self.sm.start_pos.laser_deg_offset,
                90 + self.sm.start_pos.laser_deg_offset,
                180 + self.sm.start_pos.laser_deg_offset,
                270 + self.sm.start_pos.laser_deg_offset,
            ]
        ...
        if (
            self.node.s_ranges[degrees_directions[0]] != 0
            and self.node.s_ranges[degrees_directions[0]]
            < self.highest_distance
        ):
            self.sm.laser_distance.d0 = self.node.s_ranges[
                degrees_directions[0]
            ]
        else:
            self.sm.laser_distance.d0 = self.highest_distance

        if (
            self.node.s_ranges[degrees_directions[1]] != 0
            and self.node.s_ranges[degrees_directions[1]]
            < self.highest_distance
        ):
            self.sm.laser_distance.d90 = self.node.s_ranges[
                degrees_directions[1]
            ]
        else:
            self.sm.laser_distance.d90 = self.highest_distance

        if (
            self.node.s_ranges[degrees_directions[2]] != 0
            and self.node.s_ranges[degrees_directions[2]]
            < self.highest_distance
        ):
            self.sm.laser_distance.d180 = self.node.s_ranges[
                degrees_directions[2]
            ]
        else:
            self.sm.laser_distance.d180 = self.highest_distance

        if (
            self.node.s_ranges[degrees_directions[3]] != 0
            and self.node.s_ranges[degrees_directions[3]]
            < self.highest_distance
        ):
            self.sm.laser_distance.dm90 = self.node.s_ranges[
                degrees_directions[3]
            ]
        else:
        ...
        list_of_tuples_min = [
            (
                (self.node.s_ranges[i], self.degrees_list[i])
                if self.node.s_ranges[i] != 0
        ...
        list_of_tuples_max = [
            (
                (self.node.s_ranges[i], self.degrees_list[i])
                if self.node.s_ranges[i] < self.highest_distance
        ...
        list_for_mean = [
            x
            for x in self.node.s_ranges
            if x != 0 and x < self.highest_distance
        ]

        # Get the min, mean and max of all lasers and in what direction the min and max are
        self.sm.laser_distance.dmin, self.sm.laser_distance.min_deg = min(
            list_of_tuples_min
        )
        ...
        if (
            ...
            self.sm.laser_distance.min_deg = (
                self.sm.laser_distance.min_deg
                - self.sm.start_pos.laser_deg_offset
                + 360
            )
        elif (
            self.sm.laser_distance.min_deg - self.sm.start_pos.laser_deg_offset
        ...
        self.sm.laser_distance.dmax, self.sm.laser_distance.max_deg = max(
            list_of_tuples_max
        )
        ...
        if (
            ...
            self.sm.laser_distance.max_deg = (
                self.sm.laser_distance.max_deg
                - self.sm.start_pos.laser_deg_offset
                + 360
            )
        ...
        temp_list = temp_list[
            60
            - range_front
            + self.sm.start_pos.laser_deg_offset : 60
            + range_front
            + self.sm.start_pos.laser_deg_offset
        ]
        temp_list2 = [
            (x, y - self.sm.start_pos.laser_deg_offset)
        ...
        if temp_list2 != []:
            (
                self.sm.laser_distance.dfront_min,
                self.sm.laser_distance.min_deg_f,
            ) = min(temp_list2)
            (
                self.sm.laser_distance.dfront_max,
                self.sm.laser_distance.max_deg_f,
            ) = max(temp_list2)
        ...
        temp_list = list_of_tuples_min[
            270
            - range_right
            + self.sm.start_pos.laser_deg_offset : 270
            + range_right
            + self.sm.start_pos.laser_deg_offset
        ]
        temp_list2 = [
        ...
        if temp_list2 != []:
            (
                self.sm.laser_distance.dright_min,
                self.sm.laser_distance.min_deg_r,
            ) = min(temp_list2)
            (
                self.sm.laser_distance.dright_max,
                self.sm.laser_distance.max_deg_r,
            ) = max(temp_list2)
        ...
        temp_list = list_of_tuples_min[
            180
            - range_back
            + self.sm.start_pos.laser_deg_offset : 180
            + range_back
            + self.sm.start_pos.laser_deg_offset
        ]
        temp_list2 = [
        ...
        if temp_list2 != []:
            (
                self.sm.laser_distance.dback_min,
                self.sm.laser_distance.min_deg_b,
            ) = min(temp_list2)
            ...
            (
                self.sm.laser_distance.dback_max,
                self.sm.laser_distance.max_deg_b,
            ) = max(temp_list2)
            if (
                self.sm.laser_distance.max_deg_b
                - self.sm.start_pos.laser_deg_offset
        ...
        temp_list = list_of_tuples_min[
            90
            - range_left
            + self.sm.start_pos.laser_deg_offset : 90
            + range_left
            + self.sm.start_pos.laser_deg_offset
        ]
        temp_list2 = [
        ...
        if temp_list2 != []:
            (
                self.sm.laser_distance.dleft_min,
                self.sm.laser_distance.min_deg_l,
            ) = min(temp_list2)
            (
                self.sm.laser_distance.dleft_max,
                self.sm.laser_distance.max_deg_l,
            ) = max(temp_list2)
        ...
        self.sm.laser_intensity.i0 = self.node.s_intensity[
            0 + self.sm.start_pos.laser_deg_offset
        ]
        self.sm.laser_intensity.i90 = self.node.s_intensity[
            90 + self.sm.start_pos.laser_deg_offset
        ]
        self.sm.laser_intensity.i180 = self.node.s_intensity[
            180 + self.sm.start_pos.laser_deg_offset
        ]
        self.sm.laser_intensity.im90 = self.node.s_intensity[
            270 + self.sm.start_pos.laser_deg_offset
        ]
        ...
        temp_list = self.node.s_intensity[
            270
            - range_right
            + self.sm.start_pos.laser_deg_offset : 270
            + range_right
    ...
    def parse_input(self):
        if self.input == "m":
            ...
            return True
        elif len(self.input) > 0 and self.input[0] == "p":
            args = self.input.split()
            _, x, y = args
            x = int(x)
            y = int(y)
            if x < 0 or x > 3:
                print(f"Error with x value for p(oint) command: {x},{y}")
            if y < 0 or y > 3:
                print(f"Error with y value for p(oint) command: {x},{y}")
            self.sm.internal_operation_callback.target = Coord(x, y)
            self.sm.internal_operation_callback.current_grid = self.maze.grid
            self.sm.computer.raise_p_press()
        else:
            return False
    ...
    def set_zero(self):
        ...
        if self.sm.laser_distance.min_deg < -135:
            self.sm.start_pos.laser_deg_offset = (
                self.sm.laser_distance.min_deg + 180
            )
        elif self.sm.laser_distance.min_deg < -45:
            self.sm.start_pos.laser_deg_offset = (
                self.sm.laser_distance.min_deg + 90
            )
        elif self.sm.laser_distance.min_deg < 45:
    ...
    def receive_grid(self):
        ...
        self.sm.grid.wall_front = self.current_node.walls[
            self.sm.grid.orientation
        ]
        self.sm.grid.wall_right = self.current_node.walls[
            (self.sm.grid.orientation + 1) % 4
        ]
        self.sm.grid.wall_left = self.current_node.walls[
            (self.sm.grid.orientation - 1) % 4
        ]
        self.sm.grid.wall_back = self.current_node.walls[
            (self.sm.grid.orientation - 2) % 4
        ]
    ...
    def update_grid(self):
        ...
        self.current_node.walls[self.sm.grid.orientation] = (
            self.sm.grid.wall_front
        )
        self.current_node.walls[(self.sm.grid.orientation + 1) % 4] = (
            self.sm.grid.wall_right
        )
        self.current_node.walls[(self.sm.grid.orientation - 1) % 4] = (
            self.sm.grid.wall_left
        )
