...
class Timer(ITimer):
    ...
    def set_timer(self, callback, event_id, interval, periodic):
        ...
        if periodic is False:
            self.timer_queue[timer_id] = SingleTimer(
                m_interval, callback, event_id
            )
        else:
            self.timer_queue[timer_id] = RepeatedTimer(
                m_interval, callback, event_id
            )
...
class SingleTimer:
    ...
    def __init__(self, period, callback, event_id):
        ...
        self.single_timer = threading.Timer(
            period, self.callback.time_elapsed, [self.event_id]
        )
...
class RepeatedTimer:
    ...
    def stop(self):
        # We edited this function to avoid errors where threads were joined
        # while running themselves
        self.event.set()
        if (
            ...
            print("Thread is either None or trying to join itself!")
